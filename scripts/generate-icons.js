...
async function generateIcons() {
  ...
  if (!fs.existsSync(sourceImagePath)) {
    ...
    console.log('Please ensure paljs-icon-square.png exists in the public directory');
    return;
  }
  ...
  try {
    ...
    for (const { size, name } of iconSizes) {
      const outputPath = name === 'apple-touch-icon.png' ? path.join(publicDir, name) : path.join(iconsDir, name);
    ...
    for (const { size, name } of iconSizes) {
      ...
      const outputPath =
        name === 'apple-touch-icon.png' ? path.join(publicDir, `apple-touch-icon.svg`) : path.join(iconsDir, svgName);
