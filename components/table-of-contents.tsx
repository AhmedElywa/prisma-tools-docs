...
import { type TocEntry, useExtractHeadings, useTableOfContents } from '@/hooks/use-toc';
...
const TocItem = memo(function TocItem({ heading, isActive }: { heading: TocEntry; isActive: boolean }) {
  return (
    <li
...
export const TableOfContents = memo(function TableOfContents({ headings: providedHeadings }: TableOfContentsProps) {
      ...
      <div className="space-y-2">
        ...
        <ul className="m-0 list-none">
          {filteredHeadings.map((heading) => (
            <TocItem key={heading.slug} heading={heading} isActive={heading.slug === activeId} />
          ))}
        </ul>
      </div>
    </div>
