...
export function useMDXComponents(components: MDXComponents): MDXComponents {
  return {
    ...
    h2: ({ children }) => (
      <h2 className="mt-10 scroll-m-20 border-b pb-2 text-3xl font-semibold tracking-tight first:mt-0">{children}</h2>
    ),
    ...
    pre: ({ children, ...props }) => {
      ...
      return (
        <pre className="my-6 overflow-x-auto rounded-lg border bg-muted p-4 text-sm" {...props}>
          {children}
        </pre>
      )
    },
    ...
    Callout: ({
      ...
      children,
      type = "default",
    }: { title?: string; children: React.ReactNode; type?: "default" | "warning" | "danger" }) => (
      <Alert
        className={cn(
          "my-6",
          type === "warning" &&
    ...
    ReactDemo: () => <div className="my-6 p-4 border rounded-lg bg-background">Embedded React Demo Placeholder</div>,
    ...components,
  }
}
